...
if [[ -n "$TAILSCALE_IP" ]]; then
    ...
    echo ""
    section_break
    echo -e "${BOLD}COMPLETE THE 3 STEPS ABOVE, THEN INSTALL CLIENT${NC}"
    echo ""
    echo "Next: Install the client on your laptop/desktop:"
    ...
    echo ""
    echo "After both server and client are set up, test the connection:"
    echo -e "  ${BLUE}curl http://private-ai-server:11434/v1/models${NC}"
    echo ""
    echo "Troubleshooting (if needed):"
    echo -e "  • If Ollama stops responding: ${BLUE}launchctl kickstart -k $LAUNCHD_DOMAIN/$LAUNCHD_LABEL${NC}"
    echo -e "  • View error logs: ${BLUE}tail -f /tmp/ollama.stderr.log${NC}"
    echo ""
else
