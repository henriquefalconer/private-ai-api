...
if [[ "$SERVER_REACHABLE" == "true" ]]; then
    ...
    echo ""
    echo -e "Your server currently has ${GREEN}1 model${NC} loaded."
    echo ""
    echo "List available models:"
    echo -e "  ${YELLOW}curl http://$SERVER_HOSTNAME:11434/v1/models${NC}"
    echo ""
    echo -e "${BLUE}Note:${NC} Models are managed on the server only."
    echo "To add models:"
    echo -e "  1. SSH to server and run: ${YELLOW}ollama pull model-name${NC}"
    echo -e "  2. Then select in Aider: ${GREEN}aider --model ollama/model-name${NC}"
else
